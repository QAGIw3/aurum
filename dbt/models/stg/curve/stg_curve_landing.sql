...
select
    ...
    asset_class,
    region,
    coalesce(upper(nullif(iso, '')), 'UNKNOWN') as iso,
    location,
    coalesce(upper(nullif(market, '')), 'UNKNOWN') as market,
    coalesce(upper(nullif(product, '')), 'UNKNOWN') as product,
    coalesce(upper(nullif(block, '')), 'UNKNOWN') as block,
    spark_location,
    price_type,
    units_raw,
    currency,
