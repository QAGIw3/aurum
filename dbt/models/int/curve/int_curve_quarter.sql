...
select
    ...
    bid,
    ask,
    lower(to_hex(md5(to_utf8(curve_key || '|quarter|' || cast(year(contract_month) as varchar) || 'Q' || cast(quarter(contract_month) as varchar) || '|' || cast(asof_date as varchar))))) as version_hash,
    _ingest_ts
from quarterly;
