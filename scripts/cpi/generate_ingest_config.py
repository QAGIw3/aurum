#!/usr/bin/env python3
"""Generate CPI ingest configuration from catalog for Airflow DAGs."""
...
# Schema for CPI ingest dataset configuration
...
@dataclass
class CpiSeries:
    """CPI series metadata from catalog."""
    id: str
    title: str
    units: str
    frequency: str
    ...
    @property
    def area(self) -> str:
        """Extract area information from CPI series title."""
        return _extract_area_from_title(self.title)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "CpiSeries":
        return cls(
            id=data["id"],
            title=data["title"],
            units=data["units"],
...
class GeneratedDataset:
    """Generated CPI ingest dataset configuration."""
    source_name: str
    series_id: str
    description: str
    schedule: str
...
def _load_cpi_catalog(catalog_path: Path) -> List[CpiSeries]:
    """Load CPI catalog from JSON file."""
    try:
        ...
        for series_data in data.get("series", []):
            series.append(CpiSeries.from_dict(series_data))

        return series
    except Exception as e:
        raise RuntimeError(f"Failed to load CPI catalog from {catalog_path}: {e}")


def _frequency_to_schedule(frequency: str) -> str:
    """Convert CPI frequency to cron schedule."""
    freq_mapping = {
        "Daily": "0 6 * * *",
        "Weekly": "0 6 * * 1",
        "Monthly": "0 6 1 * *",
...
def _frequency_to_label(frequency: str) -> str:
    """Convert CPI frequency to label."""
    freq_mapping = {
        "Daily": "DAILY",
        "Weekly": "WEEKLY",
        "Monthly": "MONTHLY",
...
def _extract_area_from_title(title: str) -> str:
    """Extract area information from CPI series title."""
    # Check for most specific patterns first
    if "U.S. City Average" in title or ("Urban" in title and "Average" in title):
        return "US_CITY_AVERAGE"
    if "United States" in title or "U.S." in title:
        return "US"
    return "UNKNOWN"


def _build_generated_entry(series: CpiSeries, *, default_schedule: str) -> GeneratedDataset:
    """Build generated dataset configuration from CPI series."""
    source_name = f"cpi_{series.id.lower()}"
    frequency_label = _frequency_to_label(series.frequency)
    area = _extract_area_from_title(series.title)
    ...
    return GeneratedDataset(
        ...
        default_topic=f"aurum.ref.cpi.{series.id.lower()}.v1",
...
def _write_schema_file() -> None:
    ...
    schema_path = config_dir / "cpi_ingest_datasets.schema.json"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Generate CPI ingest configuration")
    parser.add_argument("--catalog", type=Path, default=REPO_ROOT / "config" / "cpi_catalog.json",
                       help="Path to CPI catalog JSON file")
    parser.add_argument("--output", type=Path, default=REPO_ROOT / "config" / "cpi_ingest_datasets.json",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        # Load catalog
        series = _load_cpi_catalog(args.catalog)
        print(f"Loaded {len(series)} CPI series from catalog")
        ...
        if args.export_vars:
            ...
            print("airflow variables set aurum_cpi_catalog_url 'https://api.stlouisfed.org/fred'")
            print("airflow variables set aurum_cpi_api_base 'https://api.stlouisfed.org/fred'")
