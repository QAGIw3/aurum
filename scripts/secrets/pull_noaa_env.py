#!/usr/bin/env python3
"""Pull NOAA environment variables from Vault for local development."""
...
DEFAULT_ENV_MAPPING = {
    ...
    "base_url": "NOAA_GHCND_BASE_URL",
    "dataset": "NOAA_GHCND_DATASET",
    "limit": "NOAA_GHCND_LIMIT",
    "timeout": "NOAA_GHCND_TIMEOUT",
    "station_limit": "NOAA_GHCND_STATION_LIMIT",
    "unit_code": "NOAA_GHCND_UNIT_CODE",
    "units": "NOAA_GHCND_UNITS"
}


def pull_noaa_env_from_vault(
    env_mapping: dict[str, str] | None = None,
    vault_addr: str = VAULT_ADDR,
    vault_token: str = VAULT_TOKEN
) -> dict[str, str]:
    """Pull NOAA environment variables from Vault."""
    ...
    try:
        ...
        raise RuntimeError(f"Failed to pull NOAA secrets from Vault: {e}")
...
def save_to_env_file(env_vars: dict[str, str], filepath: str) -> None:
    ...
    with open(filepath, 'w') as f:
        f.write("# NOAA Environment Variables\n")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Pull NOAA environment variables from Vault")
    ...
    try:
        env_vars = pull_noaa_env_from_vault(
            env_mapping=DEFAULT_ENV_MAPPING,
        ...
        if not env_vars:
            print("No NOAA environment variables found in Vault", file=sys.stderr)
            return 1

        if args.dry_run:
            print("# NOAA Environment Variables (dry run)")
            for key, value in env_vars.items():
                print(f"# {key}={value}")
            return 0
