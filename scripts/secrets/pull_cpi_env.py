#!/usr/bin/env python3
"""Pull CPI environment variables from Vault for local development."""
...
DEFAULT_ENV_MAPPING = {
    ...
    "base_url": "CPI_BASE_URL",
    "series_id": "CPI_SERIES_ID",
    "frequency": "CPI_FREQUENCY",
    "seasonal_adj": "CPI_SEASONAL_ADJ",
    "area": "CPI_AREA",
    "units": "CPI_UNITS",
    "source": "CPI_SOURCE"
}
...
def pull_cpi_env_from_vault(
    env_mapping: dict[str, str] | None = None,
    vault_addr: str = VAULT_ADDR,
    vault_token: str = VAULT_TOKEN
) -> dict[str, str]:
    """Pull CPI environment variables from Vault."""
    ...
    try:
        ...
        raise RuntimeError(f"Failed to pull CPI secrets from Vault: {e}")
...
def save_to_env_file(env_vars: dict[str, str], filepath: str) -> None:
    ...
    with open(filepath, 'w') as f:
        f.write("# CPI Environment Variables\n")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Pull CPI environment variables from Vault")
    ...
    try:
        env_vars = pull_cpi_env_from_vault(
            env_mapping=DEFAULT_ENV_MAPPING,
        ...
        if not env_vars:
            print("No CPI environment variables found in Vault", file=sys.stderr)
            return 1

        if args.dry_run:
            print("# CPI Environment Variables (dry run)")
            for key, value in env_vars.items():
                print(f"# {key}={value}")
            return 0
